...
def harmonize_range(image):
    """
    Assuring that all Image have values within [0, 255]
    input:
...
def synthetization_process_constraints(u_path, v_path, num_segments, r, eps, n_iter, n_neighbours, l, rho, k, opt_threshold, tau, opt_iter):
    ...
    histo = match_histograms(u, v, multichannel=True)
    ...
    plt.imshow(u)
    plt.imsave("Results/Modify/original.jpg",u.astype(np.uint8))
    plt.subplot(1, 2, 2)
    plt.title('Match Histogram')
    plt.imshow(histo)
    plt.imsave("Results/Modify/histo.jpg",histo.astype(np.uint8))
    plt.tight_layout()
    plt.savefig('Results/matchhistogram.png')
    ...
    plt.imshow(u)
    plt.subplot(2, 2, 2)
    plt.title('SP segmentation of input image')
    plt.imshow(superpixels_u)
    plt.subplot(2, 2, 3)
    plt.title('"Color-Palette" Image')
    plt.imshow(v)
    plt.subplot(2, 2, 4)
    plt.title('SP segmentation "color-palette" image')
    plt.imshow(superpixels_v)
    plt.tight_layout()
    plt.savefig('Results/superpixel.png')
    ...
    plt.figure()
    plt.subplot(2, 2, 1)
    plt.title('SP segmentation input image')
    plt.imshow(superpixels_u)
    plt.subplot(2, 2, 2)
    plt.title('SP segmentation "color-palette" image')
    plt.imshow(superpixels_v)
    plt.subplot(2, 2, 3)
    plt.title('SP color transfer (GUROBI library)')
    plt.imshow(superpixels_w1)
    plt.tight_layout()
    plt.savefig('Results/gurobitransferSP.png')
    plt.imsave("Results/Modify/SP_transfer.jpg",superpixels_w1.astype(np.uint8))

    #P = optimal_transport.projection_matrix(X, Y, mu, nu)
    #P = optimal_transport.gradient_descent(X, Y, P, W, C, mu, nu, l, rho, k, n_iter, eps, tau)
    #P = optimal_transport.gradient_descent(X, Y, P, W, C, mu, nu, l, rho, k, opt_iter, opt_threshold, tau)
    superpixels_w = image_reconstruction.superpixels_transfer(P, mu, Y, segments_u)

    plt.figure()
    plt.title('Superpixel transfer others')
    plt.subplot(1, 3, 1)
    plt.imshow(superpixels_u)
    plt.imsave("Results/Modify/SP_u.jpg",superpixels_u.astype(np.uint8))
    plt.subplot(1, 3, 2)
    plt.imshow(superpixels_v)
    plt.imsave("Results/Modify/SP_v.jpg",superpixels_v.astype(np.uint8))
    plt.subplot(1, 3, 3)
    plt.imshow(superpixels_w)
    ...
    plt.imshow(w)
    plt.subplot(1, 3, 3)
    plt.title('Post processing C-T Image')
    plt.imshow(w_filtered_final)
    plt.tight_layout()
    plt.savefig('Results/postprocessing.png')
    ...
    plt.imshow(w_filtered_final)
    plt.imsave("Results/Modify/FinalImage.jpg",w_filtered_final.astype(np.uint8))
    plt.tight_layout()
    plt.savefig('Results/finalresults.png')
    #plt.show()
    print("TYPEW",type(w_filtered_final))
    print("TYPE u",type(u))
    max,min = np.max([np.amax(w_filtered_final),np.amax(u)]), np.min([np.amin(w_filtered_final),np.amin(u)])
    print("MAX",max)
    ssim_ = ssim(u, w_filtered_final,
                  data_range=max - min,multichannel=True)
    mse = mean_squared_error(u,w_filtered_final)
    d = 2**8-1
